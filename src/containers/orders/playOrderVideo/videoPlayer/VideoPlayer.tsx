...
import { FullscreenEnterIcon, FullscreenExitIcon, PauseIcon, PlayIcon, ReplayIcon, Spinner } from './icons/icons';

const IOS_TRACK_IMAGE = require('./icons/track.png');
const IOS_THUMB_IMAGE = require('./icons/thumb.png');
...
type GetDefaultProps<P extends object, DP extends Partial<P> = Partial<P>> = DP & Diff<P, DP>;

export const createPropsGetter = <DP extends Readonly<object>>(_defaultProps: DP) => <P extends Partial<DP>>(
  props: P,
): GetDefaultProps<P, DP> => props as any;
...
const defaultProps = {
  ...
  errorCallback: (error: IError) => console.error('Error: ', error.message, error.type, error.obj) as void,
  switchToLandscape: () =>
...
type Props = { isPortrait: boolean } & Partial<DefaultProps>;
...
export class VideoPlayer extends React.Component<Props, State> {
  ...
  public constructor(props: Props) {
    ...
    this.state = {
      ...
      error: '',
      // Controls display state
      controlsOpacity: showControlsOnLoad ? 1 : 0,
  ...
  private playbackCallback = (status: PlaybackStatus) => {
    ...
    if (!status.isLoaded) {
      ...
      if (this.state.seekState === SeekStates.NotSeeking && this.state.playbackState !== PlaybackStates.Ended) {
  ...
  private getSeekSliderPosition = () => {
    if (this.playbackInstance && this.state.playbackInstancePosition && this.state.playbackInstanceDuration) {
  ...
  private onSeekSliderValueChange = async () => {
    if (this.playbackInstance !== null && this.state.seekState !== SeekStates.Seeking) {
      ...
      this.shouldPlayAtEndOfSeek =
        this.state.seekState === SeekStates.Seeked ? this.shouldPlayAtEndOfSeek : this.state.shouldPlay;
      // Pause the video
  ...
  private onSeekSliderSlidingComplete = async (value: number) => {
    ...
    if (this.playbackInstance !== null) {
      ...
      this.setPlaybackState(this.shouldPlayAtEndOfSeek ? PlaybackStates.Buffering : PlaybackStates.Paused);
      try {
  ...
  private showControls = () => {
    ...
    this.showingAnimation = Animated.timing(new Animated.Value(this.state.controlsOpacity), {
      toValue: 1,
  ...
  private hideControls = (immediate = false) => {
    ...
    this.hideAnimation = Animated.timing(new Animated.Value(this.state.controlsOpacity), {
      toValue: 0,
  ...
  private resetControlsTimer = () => {
    ...
    this.controlsTimer = setTimeout(() => this.onTimerDone(), hideControlsTimerDuration) as unknown as number;
  };
  ...
  render() {
    ...
    const { style, onPlaybackStatusUpdate, source, ...otherVideoProps } = videoProps;
